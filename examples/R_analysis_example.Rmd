...
Set up the environment with `NumPy` or specify a path to the corresponding Python executable. **NOTE** replace `data_analysis`/`my_env` with your real environment name.
```{r}
...
And let's check out the image we will analyse.
...
Run `fastLBP` right on the image without using tissue mask.
...
You can already explore individual features and notice outlines of the most prominent elements on the image.
```{r, fig.width=5, fig.height=5}
image(lbp_res[, , 2])
```
...
So we have PCA projections with 15 retained components and 2D UMAP embeddings.
```{r}
...
clustering_img <- clustering_to_2d_img(kmeans_pca_labels, dim(lbp_res)[1],
                                       dim(lbp_res)[2])
plot_2d_img_with_colorbar(clustering_img, main = "PCA result")


clustering_img_umap <- clustering_to_2d_img(kmeans_umap_labels, dim(lbp_res)[1],
                                       dim(lbp_res)[2])
plot_2d_img_with_colorbar(clustering_img_umap, main = "UMAP result")
...
Wee need to define some helper functions to obtain tissue mask from the image using Otsu thresholding. Then we will create the mask and run fastlbp using it.
```{python}
...
You can take a look on the tissue mask that we just got. This gives you the information about what region of the original image was subjected to the LBP calculation. Only patches from the masked region (marked with `True` or `1`) will be used in the downstream analysis as well.
```{r, fig.width=8, fig.height=5}
tissue_mask <- py$tissue_mask
# image(tissue_mask)
plot_2d_img_with_colorbar(tissue_mask)
```

Load the new result and repeat the same analysis we carried out earlier in the no-mask scenario. However, before proceeding we need to get patch mask from our pixel mask to work with patches rather than with pixels. Essentially, we just need to downscale the mask.
...
Notice how patch mask is more pixelated than the pixel mask because of the downscaling.
```{r, fig.width=8, fig.height=5}
tissue_patch_mask <- py$tissue_patch_mask
tissue_patch_mask_uint8 <- py$tissue_patch_mask_uint8
# image(tissue_mask)
plot_2d_img_with_colorbar(tissue_patch_mask_uint8, main = "Binary tissue mask")
```

Read the LBP result obtained using mask.
...
Let's examine the resulting segmentation using our helper function that supports visualization of masked arrays.
```{r, fig.width=8, fig.height=5}
# Plot PCA result
clustering_img <- clustering_to_2d_img(kmeans_pca_labels, dim(lbp_res)[1],
                                       dim(lbp_res)[2],
